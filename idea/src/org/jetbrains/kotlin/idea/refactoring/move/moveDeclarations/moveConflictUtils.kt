...
class MoveConflictChecker(
    ...
    private fun <T> unused(t: T) {
    }

    private fun checkNameClashes(conflicts: MultiMap<PsiElement, String>) {
        fun getDeclarationSignature(descriptor: DeclarationDescriptor): Any = when (descriptor) {
            is FunctionDescriptor -> descriptor//is FunctionDescriptor -> FlatSignature.createFromCallableDescriptor(descriptor)
        ...
        for (declaration in elementsToMove - doNotGoIn) {
            ...
            val declarationSignature = getDeclarationSignature(declarationDescriptor)
            if (declaration is PsiNamedElement) {
                val targetPsi = moveTarget.getOrCreateTargetPsi(declaration)
                if (targetPsi is KtDeclarationContainer) {
                    val descriptors = targetPsi.declarations
                        .map { Pair(it, it.analyze()[BindingContext.DECLARATION_TO_DESCRIPTOR, it]) }
                        .filter { it.second != null }
                    descriptors.forEach { (decl, descriptor) ->
                        val signature = getDeclarationSignature(descriptor!!)
                        if (equivalent(signature, declarationSignature)) {
                            conflicts.putValue(
                                declaration,
                                "Following declarations would clash:\n${declaration.getText()} (original scope) \n${decl.getText()} (target scope)"
                            )
                        }
                    }
                }
