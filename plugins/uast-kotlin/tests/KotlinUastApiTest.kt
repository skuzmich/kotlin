...
//fun RunAll.assertNull(any: Any?): RunAll = append(ThrowableRunnable { kotlin.test.assertNull(any) })
//fun RunAll.assertEquals(expected: String?, actual: String?): RunAll = append(ThrowableRunnable { TestCase.assertEquals(expected, actual) })

fun runAll(vararg asserts: () -> Unit): Unit {
    RunAll(*asserts.map { ThrowableRunnable<Throwable>(it) }.toTypedArray()).run()
}